...
void read_data( int* choice )
{
	...
	cmd( "button .da.b.comp -width $butWid -text Compare -command {set choice 101} -underline 1" );
	cmd( "button .da.b.abmat -width $butWid -text \"Abmat Stats\" -command {set choice 102} -underline 1" );
	...
	cmd( "pack .da.b.ts .da.b.dump .da.b.sv .da.b.sp .da.b.st .da.b.abmat .da.b.comp .da.b.fr .da.b.lat -padx 10 -pady 10 -side left -expand no -fill none" );
	...
	while ( true ) {
		...
		*choice = 0;
		...
		// Statistics
		case 12:
			statistics( choice, 0 );//no abmat stats
			cmd( "wm deiconify .log; raise .log .da" );
...
void statistics( int* choice, int abmat)
{
	...
	if (abmat == 1) { //abmat compare
		...
		if (abmat == 2) { //abmat 8 stats
			...
			{
				cmd( ".log.text.text.internal insert end \"\n\nTime series Abmat statistics:\n\n\" tabel" );
			}

			sprintf( str1, "%d Cases", max_c - min_c + 1 );
			sprintf( longmsg, "%-20s\tMin\tp05\tp25\tp50\tp75\tp95\tMax\tavg\tSDev\tMAE\tLcv\tLsk\tLku\n", str1 );
			cmd( ".log.text.text.internal insert end \"%s\" tabel", longmsg );
		}
		...
		if (abmat == 1) {
			for ( j = min_c; j <= max_c; ++j ) {
				if ( j >= start[ i ] && j <= end[ i ] && is_finite( data[ i ][ j ] ) ) {  // ignore NaNs
					//  sprintf(msg,"\n %g ",data[i][j]);plog(msg);
					if (i == 0) {
						series1.push_back(data[i][j]);
					}
					else
						if (i == 1) {
							series2.push_back(data[i][j]);
						}
			...
			if (abmat == 2) {
				...
				auto eightStats = abmat_stats(series1);
				series1.clear();
				sprintf( msg, "%s %s ", str[ i ], tag[ i ] );
				sprintf( str1, "%-20s\t", msg );
				cmd( ".log.text.text.internal insert end \"%s\" tabel", str1 );
				sprintf( longmsg, "%.*g\t%.*g\t%.*g\t%.*g\t%.*g\t%.*g\t%.*g\t%.*g\t%.*g\t%.*g\t%.*g\t%.*g\t%.*g\n",
								 pdigits, eightStats[astat_min], pdigits, eightStats[astat_p05], pdigits,
								 eightStats[astat_p25], pdigits, eightStats[astat_p50], pdigits,
								 eightStats[astat_p75], pdigits, eightStats[astat_p95], pdigits,
								 eightStats[astat_max], pdigits, eightStats[astat_avg], pdigits,
								 eightStats[astat_sd], pdigits, eightStats[astat_mae], pdigits,
								 eightStats[astat_Lcv], pdigits, eightStats[astat_Lsk], pdigits,
								 eightStats[astat_Lku]);
				cmd( ".log.text.text.internal insert end \"%s\" tabel", longmsg );
			}//else if end
	}

	if ( abmat == 0 ) { //no abmat stats
		if ( logs )
		{
		...
		for ( i = 0; i < nv; ++i ) {
			...
			if ( num > 0 ) {
				...
				sprintf( longmsg, "%.*g\t%.*g\t%.*g\t%.*g\t%.*g\n", pdigits, av, pdigits,
								 sig, pdigits, var, pdigits, ymin, pdigits, ymax);
				cmd( ".log.text.text.internal insert end \"%s\" tabel", longmsg );
			}
		}
	}
		...
		if ( abmat == 1 ) {
			auto compStats = abmat_compare(series1, series2);
			sprintf( msg, "%s %s - %s %s", str[0 ], tag[ 0 ], str[1 ], tag[1] );
			sprintf( str1, "%-20s\t", msg );
			cmd( ".log.text.text.internal insert end \"%s\" tabel", str1 );
