...
Tabmat abmat_toVar_type(object* toVar)
{
	...
	if ( strcmp(toVar->label, lmicro) == 0 ) {
		...
		if ( strcmp(toVar->label, lmacro) == 0 ) {
			type = a_macro;
		}
		else
			if ( strcmp(toVar->label, lcond) == 0 ) {
				type = a_cond;
			}
			else
				if ( strcmp(toVar->label, lfact) == 0 ) {
					type = a_fact;
				}
				else
					if ( strcmp(toVar->label, lpstat) == 0 ) {
						type = a_pstat;
					}
					else
						if ( strcmp(toVar->label, lpmic) == 0 ) {
							type = a_pmic;
						}
						else
							if ( strcmp(toVar->label, lpmac) == 0 ) {
								type = a_pmac;
							}
							else
								if ( strcmp(toVar->label, lfmic) == 0 ) {
									type = a_fmic;
								}
								else
									if ( strcmp(toVar->label, lfmac) == 0 ) {
										type = a_fmac;
									}
									else
										if ( strcmp(toVar->label, lpLSD) == 0 ) {
											type = a_pLSD;
										}
										else {
