														 ...
														 bool no_search, object* maxLevel)
{
	...
	if (cv != NULL)
	{
		...
		if ( no_search ) // stop if search is disabled
		{
			return NULL;
		}
	...
	for ( cb = b, cv = NULL; cb != NULL; cb = cb->next ) {
		...
		if ( cb->head != NULL && ( caller == NULL || strcmp( cb->head->label, caller->label ) ) ) {
			cv = cb->head->search_var(this, lab, no_error, no_search, this);
		...
		if (maxLevel != NULL && cb->head == maxLevel) {
			break;
		}
	}

	if (maxLevel != NULL && this == maxLevel) {
		return NULL;
	}
...
void object::xStats_all(char const* lab, double* r, int lag)
{
	...
	// gatherData_all_cnd(Data, lab);
	// eightStats(Data, r);
	xStats_all_cnd(lab, r, "", "", 0.0, NULL, lag);
}

void object::xStats_all_cnd(char const* lab, double* r, char const condVarLab[],
														char const condition[], double condVal,
														object* fake_caller, int lag)
{
...
void object::eightStats(std::vector<double>& Data, double* r)
{
	...
	for (auto& element : m_stats) {
		plog("\n");
		plog(element.first.c_str());
		plog("\t");
		plog(std::to_string(element.second).c_str());
	}
